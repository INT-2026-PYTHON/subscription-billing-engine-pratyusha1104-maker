...
def compute_proration(
    ...
    credit_amount = Money(
        Decimal(str(credit_amount.amount)).quantize(Decimal("0.01")),
        credit_amount.currency
    )
        
    charge_amount = Money(
        Decimal(str(charge_amount.amount)).quantize(Decimal("0.01")),
    ...
    credit_tax = Money(
        Decimal(str(credit_tax.amount)).quantize(Decimal("0.01")),
        credit_tax.currency
    )
     
    charge_tax = Money(
        Decimal(str(charge_tax.amount)).quantize(Decimal("0.01")),
        charge_tax.currency
    )
