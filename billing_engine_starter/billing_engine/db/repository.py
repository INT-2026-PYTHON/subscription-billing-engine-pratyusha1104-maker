    ...
    def add(self, customer: Customer) -> Customer:
        ...
        with self.db.transaction() as conn:
            customer_id = q.insert_customer(
                conn, customer.name, customer.email, customer.country_code, customer.state_code
            )
        return Customer(
                id=customer_id,
                name=customer.name,
...
class PlanTierRepository:
    ...
    def add(self, plan_id: int, from_units: int, to_units: Optional[int], unit_price: Money) -> int:
        ...
        with self.db.transaction() as conn:
            tier_id = q.insert_plan_tier(conn, plan_id, from_units, to_units, unit_price.to_storage())
        return tier_id
    ...
    def list_for_plan(self, plan_id: int, currency: str) -> list[tuple[int, Optional[int], Money]]:
        ...
        tiers=[]
        for row in rows:
            tiers.append((row["from_units"],row["to_units"],Money(row["unit_price"],currency)))
        return tiers
...
class SubscriptionRepository:
    ...
    def __init__(self, db: Database) -> None:
        self.db = db
    def row_to_subscription(self, row) -> Subscription:
        return Subscription(
            id=row["id"],
            customer_id=row["customer_id"],
            plan_id=row["plan_id"],
            discount_id=row["discount_id"] if "discount_id" in row.keys() else None,
    ...
    def get(self, subscription_id: int) -> Optional[Subscription]:
        ...
        if not row:
            return None
        return self.row_to_subscription(row)
    ...
    def update_status(
        ...
        with self.db.transaction() as conn:
            q.update_subscription_status(conn, subscription_id, new_status.value, past_due_since.isoformat() if past_due_since else None)
...
class UsageRecordRepository:
    ...
    def add(self, subscription_id: int, metric: str, quantity: int) -> int:
        ...
        with self.db.transaction() as conn:
            usage_record_id = q.insert_usage_record(conn, subscription_id, metric, quantity)
...
class InvoiceRepository:
    ...
    def add(self, invoice: Invoice) -> Invoice:
        ...
        with self.db.transaction() as conn:
            invoice_id = q.insert_invoice( conn,
...
class InvoiceLineItemRepository:
    ...
    def update(self, line_item: InvoiceLineItem) -> None:
        # TODO Day 2.
        # Hint: q.update_invoice_line_item(...)
        with self.db.transaction() as conn:
            q.update_invoice_line_item(
                conn,
                line_item.id,
                line_item.kind.value,
                line_item.description,
    ...
    def delete(self, line_item: InvoiceLineItem) -> None:
        # TODO Day 2.
        # Hint: q.delete_invoice_line_item(...)
        with self.db.transaction() as conn:
            q.delete_invoice_line_item(conn, line_item.id)
